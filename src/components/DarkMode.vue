<template>
    <label class="inline-flex items-center cursor-pointer">
        ...
        <div class="w-12 h-6 bg-gray-200 rounded-full relative transition-all" :class="{ 'bg-blue-500': isDark }">
            <div class="w-6 h-6 bg-white rounded-full absolute top-0 left-0 transition-transform"
                :style="isDark ? { transform: 'translateX(100%)' } : {}">
            </div>
        </div>
        <svg v-if="isDark" class="w-6 h-6 ml-3 text-gray-800" fill="currentColor" viewBox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg">
            <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
        </svg>
        <svg v-else class="w-6 h-6 ml-3 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"
            xmlns="http://www.w3.org/2000/svg">
